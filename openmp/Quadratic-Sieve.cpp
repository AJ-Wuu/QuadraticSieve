...
inline uint64_t pow_mod(uint64_t a, uint64_t b, uint64_t m) {
    uint64_t r = 1;
    while (b > 0) {
        if (b & 1) {
            r = r * a % m;
...
inline int64_t legendre_symbol(uint64_t a, uint64_t p) {
...
inline void tonelli_shanks(uint64_t n, uint64_t p, uint64_t *result) {
    if (p == 2) {
        result[0] = n;
        result[1] = n;
        return;
...
#pragma omp parallel for shared(fb_indexes, factor_base, N, sqrt_N)
...
#pragma omp parallel for shared(row_words, factor_base, matrix)
